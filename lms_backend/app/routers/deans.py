...
@router.post("/recalculate-all-academic-results")
def recalculate_all_academic_results(
    ...
    - Xóa AcademicResult không còn enrollments (total_credits = 0)
    - Recalculate lại từ enrollments hiện có
    """
    check_dean_role(current_user)
    from app.services import dean_service
    return dean_service.recalculate_all_academic_results(db)
...
def update_class(
    ...
    if student_ids:
        ...
        # Nếu semester thay đổi, cần recalculate cả 2 semester (cũ và mới)
        ...
        # Luôn recalculate semester hiện tại (mới) khi có thay đổi
        ...
        for student_id in student_ids:
            for semester_id in semesters_to_recalculate:
                try:
                    ...
                    print(f"Error recalculating semester result for student {student_id}, semester {semester_id}: {e}")
...
def delete_class(
    ...
    # Lấy danh sách students đã enroll và semester
    semester_code = db_class.semester
    enrollments = db.query(Enrollment).filter(Enrollment.class_id == class_id).all()
    student_ids = [e.student_id for e in enrollments]
    enrollment_ids = [e.id for e in enrollments]
    
    print(f"\n🗑️  Deleting class {db_class.code} (ID: {class_id})")
    print(f"   Semester: {semester_code}")
    print(f"   Students affected: {len(student_ids)}")
    print(f"   Enrollments to delete: {len(enrollments)}")
    
    # Xóa grades trước (nếu có) - xóa theo enrollment_ids để đảm bảo xóa hết
    if enrollment_ids:
        deleted_grades = db.query(Grade).filter(Grade.enrollment_id.in_(enrollment_ids)).delete(synchronize_session=False)
        db.commit()
        print(f"   ✅ Deleted {deleted_grades} grades")
    
    # Xóa enrollments
    if enrollments:
        deleted_enrollments = db.query(Enrollment).filter(Enrollment.class_id == class_id).delete(synchronize_session=False)
        db.commit()
    ...
    # Xóa schedules nếu có
    from app.models.academic import Schedule
    db.query(Schedule).filter(Schedule.class_id == class_id).delete(synchronize_session=False)
    db.commit()
    ...
    # Expire tất cả objects trong session để đảm bảo query dữ liệu mới nhất
    db.expire_all()
    
    # Verify: Kiểm tra không còn enrollments nào của lớp đã xóa
    remaining_enrollments = db.query(Enrollment).filter(Enrollment.class_id == class_id).count()
    if remaining_enrollments > 0:
        print(f"   ⚠️  WARNING: Still found {remaining_enrollments} enrollments for deleted class!")
    else:
        print(f"   ✅ Verified: No remaining enrollments for deleted class")
    
    # Recalculate semester results cho tất cả students bị ảnh hưởng
    # QUAN TRỌNG: Phải recalculate SAU KHI đã xóa hết enrollments và grades
    if semester_code and student_ids:
        semester = db.query(Semester).filter(Semester.code == semester_code).first()
        if semester:
            print(f"\n🔄 Recalculating semester results for {len(student_ids)} students after deleting class {class_id}")
            print(f"   Semester: {semester_code} (ID: {semester.id})")
            print(f"   Students: {student_ids}")
            
            for student_id in student_ids:
                try:
                    # Expire session để đảm bảo query enrollments mới nhất (không có lớp đã xóa)
                    db.expire_all()
                    ...
                    if result:
                        ...
                        print(f"   ✅ Student {student_id}: AcademicResult deleted (no enrollments in semester)")
                except Exception as e:
                    print(f"   ❌ Error recalculating for student {student_id}: {e}")
                    import traceback
                    traceback.print_exc()
            
            print(f"✅ Completed recalculating for all {len(student_ids)} students\n")
