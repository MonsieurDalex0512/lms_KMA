...
def calculate_semester_gpa(student_id: int, semester_code: str, db: Session) -> Tuple[float, int, int, int]:
    ...
    """
    # Query enrollments với join Class để đảm bảo chỉ lấy enrollments của lớp còn tồn tại
...
def calculate_and_save_semester_result(student_id: int, semester_id: int, db: Session) -> AcademicResult:
    ...
    # Nếu không còn enrollments (total_credits = 0), xóa AcademicResult
    if total_credits == 0:
        if existing_result:
            db.delete(existing_result)
            db.commit()
            print(f"   🗑️  Deleted AcademicResult for student {student_id}, semester {semester.code} (no enrollments)")
        # Update cumulative result vì có thể ảnh hưởng đến CPA
        update_cumulative_result(student_id, db)
        # Trả về None hoặc tạo result với giá trị 0
        return None
    else:
        # Có enrollments, cập nhật hoặc tạo mới
        if existing_result:
